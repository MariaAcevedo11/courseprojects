...
<template>
  <div class="space-y-6">
    ...
    <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
      ...
      <form @submit.prevent="submitReview" class="space-y-3">
        ...
        <div>
          ...
          <textarea id="comment" v-model="form.comment" rows="3"
            class="w-full border border-gray-300 rounded py-2 px-3 focus:outline-none focus:ring focus:border-blue-300"
        ...
        <button type="submit" :disabled="isSubmitting || !form.comment.trim()"
    ...
    <!-- Review list -->
    <ul class="space-y-4">
      <li v-for="review in reviews" :key="review.id" class="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
